-- Fix matching grading: parse options that may be stored as a JSON string, and prevent empty expected-mapping from being treated as correct
  ...
  LOOP
    ...
    ELSIF v_question.question_type = 'matching' THEN
      ...
      IF v_user_answer IS NOT NULL AND v_options IS NOT NULL THEN
        -- Options may be stored as a JSON array OR as a JSON string containing that array.
        ...
        -- Prevent invalid/empty mappings from ever being graded as correct
